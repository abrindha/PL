...
let solve_constraints
  ...
  let zero_ast = Z3Real.mk_numeral_i ctx 0 in 
  let undefined_ast = zero_ast in 
  let real_sort = (Z3Real.mk_sort ctx) in
  ...
  let var_to_ast str = 
    ...
    with _ -> 
      ...
      let ast = Z3Real.mk_const_s ctx str in
  ...
  let rec exp_to_ast (exp : Cil.exp) : Z3.Expr.expr =
    ...
    *)
    ...
    | CastE(_,e) -> Z3Real.mk_real2int ctx (exp_to_ast e)
    | _ -> 
...
let emit_test_case
  ...
  let ri = Str.regexp "char \*" in
	let rf = Str.regexp "float \*" in
	let s = (Pretty.sprint ~width:80 (dn_type () formal.vtype)) in
	if (Str.string_match ri s 0) || (Str.string_match rf s 0)
	then Printf.fprintf fout "\t%s %s = malloc(200);\n" s formal.vname
	else Printf.fprintf fout "\t%s %s;\n" s formal.vname
